...
function ProjectCard({ project }) {
  ...
  return (
    <Card className={`glow-card group h-full ${project.preview ? 'pt-0' : ''}`}>
      {project.preview && (
        // The wrapper clips both the image zoom and the signal line. The
        // stretched link in CardTitle below covers the whole card, this
        // included, so the preview is not a dead zone above the title.
        ...
        <CardDescription className="flex items-center gap-2.5">
          ...
          {project.status && (
            <span className="inline-flex items-center gap-1.5 text-xs font-medium text-info">
              <span aria-hidden className="status-light size-1.5 rounded-full bg-info" />
              {project.status}
            </span>
          )}
        </CardDescription>
